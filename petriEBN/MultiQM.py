...
# Retrieved from GitHub: https://github.com/jackxiaogan/Multi-level_motif_algorithm
#-------------------------------------------------------------------------------
...
##            state = vnode.split("$",1)[1].strip()
            if node not in nodelist:
                ...
                statelist.append(fullnodestates[fullnodelist.index(cleanNode(node))])
...
##        print 'index=', fullnodelist.index(x[0][i])
        levellist.append(fullnodestates[fullnodelist.index(cleanNode(x[0][i]))])
##    print list1
